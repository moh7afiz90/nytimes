...
const ArticleCard = (props) => {
  ...
  return (
    <Card item className={(cardStyle.newsCard, shadowStyles.root)}>
      <CardMedia classes={mediaStyles} image={`http://static01.nyt.com/${props.article.multimedia[1].url}`} />
      <CardContent className={cardStyle.content}>
        <TextInfoCardContent
          classes={textCardContentStyles}
          overline={moment(props.article.pub_date).format("lll")}
          heading={props.article.headline.main}
        />
        <Button
          color={"primary"}
          fullWidth
      ...
      >
        <Fade in={open}>
           <NewsCardModal article={props.article}></NewsCardModal>
          </Fade>
        </Modal>
      </CardContent>
    </Card>
